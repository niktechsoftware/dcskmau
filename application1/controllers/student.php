<?php
	...
	function is_login(){
    	...
    	public function promotion() {
			...
			$data = Array(
    	        "pageTitle"     => ' Old Student Promotion',
    	        "smallTitle"    => '',
    	        "mainPage"      => 'Student',
    	        "subPage"       => 'Promotion',
    	        "title"         => 'Promotion',
		...
		//	print_r($stud_fsd);exit();
		   ...
		   // print_r($rty);
		   	$pcu=$dataArray["year"]="3rd";
    	    $queryString = "SELECT `fee_amount` FROM `program_subjects` WHERE `subject_name` = '$skip' AND `program_code` = '$pcu' AND `program_name` = '$rty->course'";
		...
		$this->db->where('roll_number',$rollNo);
		$rollNo=$this->db->get('student_info')->row();
			$stuFsd = $rollNo->fsd;
			$studFsd = $stuFsd+1;
			$courseEntry = $this->db->query("UPDATE `student_info` SET `fsd`= '$studFsd' WHERE 'fsd' = '$stuFsd'");
			//print_r($courseEntry);
        	if($rty->year == "3rd"):
